...
**Key Functions**: - `get_items_from_collection()` - Retrieves paginated items from collections - `get_media()` - Fetches media files associated with items - `extract_property()` - Extracts specific metadata properties - `extract_combined_values()` - Combines multiple property values
...
**Key Functions**: - `construct_payload()` - Builds DSP resource payloads - `extract_listvalueiri_from_value()` - Maps values to DSP list nodes - `specify_mediaclass()` - Determines appropriate DSP media classes
...
**Key Functions**: - `check_values()` - Compares Omeka and DSP data for differences - `sync_value()` - Synchronizes single-value properties - `sync_array_value()` - Synchronizes multi-value properties - `update_value()` - Performs actual API updates
...
**Key Functions**: - `upload_file_from_url()` - Downloads and uploads files to DSP - `create_resource()` - Creates new DSP resources - `get_full_resource()` - Retrieves complete resource data
