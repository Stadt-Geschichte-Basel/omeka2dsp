...
#### `create_resource(payload: dict, token: str) -> str`

**Purpose**: Creates a new resource in DSP using the provided payload and returns the resource IRI.
...
**Returns**: 
-   `str`: The DSP resource IRI if creation successful, empty string otherwise

**Side Effects**:

-   Creates resource in DSP
-   Logs creation success/failure with resource IRI
-   If `OMEKA_WRITEBACK_ENABLED` is true, triggers write-back of DSP URI to Omeka

**Example**:

``` python
payload = construct_payload(omeka_item, f"{PREFIX}sgb_OBJECT", project_iri, lists, "", "")
resource_iri = create_resource(payload, token)
if resource_iri:
    print(f"Created resource: {resource_iri}")
```
...
#### `update_omeka_item(item_id: int, property_id: int, dsp_uri: str) -> bool`

**Purpose**: Updates an Omeka item by adding a DSP URI to a specific Dublin Core property.

**Parameters**:

-   `item_id`: Omeka item ID to update
-   `property_id`: Dublin Core property ID (e.g., 44 for dcterms:hasVersion)
-   `dsp_uri`: DSP resource IRI to add as property value
...
1.  Fetches current item data from Omeka API
2.  Locates existing property or creates new property entry
3.  Adds DSP URI as new property value
4.  Updates item via PUT request to Omeka API

**Error Handling**:

-   Catches and logs HTTP errors
-   Returns False on any failure
-   Continues processing if write-back fails

**Example**:

``` python
success = update_omeka_item(12345, 44, "https://dsp.example.com/resource/123")
if success:
    print("DSP URI written back to Omeka successfully")
```
...
| `OMEKA_WRITEBACK_ENABLED` | string | No | Enable DSP URI write-back to Omeka | "true" |
| `DCTERMS_HASVERSION_PROPERTY_ID` | string | No | Property ID for dcterms:hasVersion | "44" |
