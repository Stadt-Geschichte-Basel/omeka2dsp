...
**Purpose**:
 Entry point that orchestrates the entire migration process.
...
**Purpose**:
 Parses command-line arguments for processing mode selection.
...
**Purpose**:
 Authenticates with DSP API and retrieves JWT token.
...
**Purpose**:
 Retrieves project information from DSP API using project shortcode.

**Parameters**:
 None (uses `PROJECT_SHORT_CODE` environment variable)
...
**Purpose**:
 Retrieves all list configurations for a DSP project.
...
**Process**:
 1. Fetches list summary from `/admin/lists/` 2. For each list, fetches detailed information from `/v2/lists/{id}` 3. Returns complete list data for mapping operations
...
**Purpose**:
 Retrieves complete resource data from DSP API.
...
**Usage**:
 Used for synchronization to compare existing DSP data with Omeka data.
...
**Purpose**:
 Finds a resource by its identifier using SPARQL query.
...
**SPARQL Query**:
 Constructs and executes a SPARQL query to find resources by identifier.
...
**Purpose**:
 Creates a new resource in DSP using the provided payload.
...
**Purpose**:
 Extracts a single property value from a DSP resource.
...
**Supported Value Types**:
 TextValue, ListValue, LinkValue, UriValue
...
**Purpose**:
 Extracts multiple values for a property from a DSP resource.
...
**Usage**:
 For properties that can have multiple values (arrays).
...
**Purpose**:
 Extracts the actual value from a DSP property entry based on its type.
...
**Purpose**:
 Converts Omeka item data into DSP-compatible JSON-LD payload.
...
**Purpose**:
 Maps an Omeka value to a DSP list node IRI.
...
**Process**:
 1. Finds the list with matching label 2. Searches through list nodes for matching value 3. Returns the node IRI for API operations
...
**Purpose**:
 Compares DSP and Omeka data to identify changes that need synchronization.
...
**Purpose**:
 Generates sync operations for single-value properties.
...
**Purpose**:
 Generates sync operations for multi-value properties.
...
**Algorithm**:
 1. Converts arrays to sets for comparison 2. Calculates additions (in Omeka but not DSP) 3. Calculates deletions (in DSP but not Omeka) 4. Generates corresponding create/delete operations


#### `update_value(token: str, item: dict, value: str, field: str, field_type: str, type_of_change: str) -> None`

**Purpose**:
 Executes a single value update operation via DSP API.
...
**Purpose**:
 Downloads a file from Omeka and uploads it to DSP storage.
...
**Process**:
 1. Downloads file from Omeka URL 2. Saves to temporary file 3. Optionally creates ZIP archive 4. Uploads to DSP via multipart form 5. Returns DSP internal filename
...
**Purpose**:
 Determines appropriate DSP media class based on MIME type.
...
**Usage**:
 Used in synchronization to detect array changes.
...
**Purpose**:
 Retrieves all items from a specified Omeka collection with pagination handling.
...
**Purpose**:
 Retrieves all media files associated with a specific Omeka item.
...
**Purpose**:
 Generic function to handle paginated API requests.
...
**Purpose**:
 Extracts a specific property value from Omeka property array.
...
**Purpose**:
 Combines text values and URI references from properties into a single array.
...
**Process**:
 1. Extracts all `@value` text fields 2. Formats URI references as HTML links 3. Escapes semicolons to prevent conflicts 4. Returns combined array
...
**Purpose**:
 Fetches project data from DSP API and saves to file.
...
**Output**:
 Saves project data to `../data/project_data.json`
...
**Purpose**:
 Fetches list summary from DSP API and saves to file.
...
**Output**:
 Saves list data to `../data/data_lists.json`
...
**Purpose**:
 Fetches complete list data for a single list ID.
...
**Process**:
 1. URL-encodes the list IRI 2. Requests detailed list data from `/v2/lists/{id}` 3. Returns complete list structure

**Main Script Logic**:
 1. Loads list summary from `data_lists.json` 2. Iterates through each list 3. Fetches detailed information for each 4. Saves all detailed lists to `data_lists_detail.json`
