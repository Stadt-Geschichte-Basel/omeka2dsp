...
Complete workflow documentation with Mermaid diagrams covering: - Main migration workflow (end-to-end process) - Data extraction workflow (Omeka API interaction) - Data transformation workflow (format conversion) - Synchronization workflow (incremental updates) - File upload workflow (media processing) - Error handling workflow (recovery and retry)

### [Python API Reference](api.qmd)

Comprehensive documentation of all 5 Python modules with 50+ functions: - `data_2_dasch.py` - Main migration orchestrator - `process_data_from_omeka.py` - Omeka API interface - `api_get_project.py` - DSP project information - `api_get_lists.py` - DSP list configuration - `api_get_lists_detailed.py` - Detailed list metadata
